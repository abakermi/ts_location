...
class BackgroundLocationService :
    ...
    companion object {
        ...
        /**
         * Requests the singleton instance of [BackgroundLocationService] or creates it, if it does
         * not yet exist.
         */
    ...
    /**
     * Context that is set once attached to a FlutterEngine. Context should no longer be referenced
     * when detached.
     */
    ...
    fun onAttachedToEngine(@NonNull context: Context, @NonNull messenger: BinaryMessenger) {
        ...
        LocalBroadcastManager.getInstance(context)
                .registerReceiver(receiver!!, IntentFilter(LocationUpdatesService.ACTION_BROADCAST))
    }
    ...
    private fun startLocationService(distanceFilter: Double?, forceLocationManager: Boolean?): Int {
        ...
        if (!bound) {
            ...
            println("startLocationService from kotlin")
        }
    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: MethodChannel.Result) {
        println("onMethodCall from kotlin ($call.method)")
        when (call.method) {
            ...
            "set_configuration" ->
                    result.success(
                            setConfiguration(call.argument<String>("interval")?.toLongOrNull())
                    )
            else -> result.notImplemented()
        }
    }

    /** Requests a location updated. If permission is denied, it requests the needed permission */
    ...
    /**
     * Requests permission for location. Depending on the current activity, displays a rationale for
     * the request.
     */
    ...
    private inner class MyReceiver : BroadcastReceiver() {
        override fun onReceive(context: Context, intent: Intent) {
            ...
            if (location != null) {
                ...
                println("method channel invoke with locationMap: $locationMap")
                ...
                channel.invokeMethod("chaheenLocation", locationMap, object : MethodChannel.Result {
                    ...
                    override fun error(errorCode: String, errorMessage: String?, errorDetails: Any?) {
                        // Handle error result
                        Log.e("MethodChannel", "Error sending location data: $errorMessage")
                    }
                
                    override fun notImplemented() {
                        // Handle case where the method has not been implemented on the other side
                        Log.e("MethodChannel", "Method not implemented.")
                    }
                })
