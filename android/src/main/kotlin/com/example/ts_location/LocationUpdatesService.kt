...
class LocationUpdatesService : Service() {
    ...
    private val notification: NotificationCompat.Builder
        ...
        get() {
            ...
            val builder =
                    NotificationCompat.Builder(this, "BackgroundLocation")
                            ...
                            .setStyle(
                                    NotificationCompat.BigTextStyle().bigText(NOTIFICATION_MESSAGE)
                            )
    ...
    override fun onCreate() {
        ...
        if (isGoogleApiAvailable && !this.forceLocationManager) {
            println("Google API is available use Fuse location")
            mFusedLocationClient = LocationServices.getFusedLocationProviderClient(this)

            mFusedLocationCallback =
                    object : LocationCallback() {
                        override fun onLocationResult(locationResult: LocationResult) {
                            println("onLocationResult called with locationResult: $locationResult")
                            // Smart cast to 'Location' is impossible, because
                            ...
                            if (newLastLocation is Location) {
                                ...
                                println("onNewLocation called with location: $newLastLocation")
                                onNewLocation(newLastLocation)
                            }
                        }
                    }
        } else {
            println("Google API is not available use LocationManager")
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            println("Create notification channel for Android Oreo and above")
    ...
    fun requestLocationUpdates() {
        ...
        try {
            if (isGoogleApiAvailable && !this.forceLocationManager) {
                println("Request location updates using FusedLocationProviderClient")
                ...
                println("Request location updates using LocationManager")
            ...
            println("Lost location permission. Could not request updates. $unlikely")
    ...
    private fun onNewLocation(location: Location) {
        ...
        println("Broadcasting location update: $location")
